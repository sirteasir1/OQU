...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `You are an IELTS Listening tutor.

Create ONE IELTS-style listening transcript (audio script) for level: ${levelLabel}.

Requirements:
- Transcript length: 280-420 words
...
Return ONLY valid JSON in this structure:
{
  ...
  "questions": [
    ...
    console.error('IELTS listening generation error:', error);
    return NextResponse.json(
      { error: 'Failed to generate IELTS listening', details: error?.message || 'Unknown error' },
      { status: 500 }
    );
  }
}
